# -*- encoding: utf-8 -*-
# @MoudleName: xscnetwork.py
# @Function : The main part of bp nerual network
# @Author : XsC
...
class CrossEntropyCost(object):
    ...
    def delta(z, a, y):
        """
        从输出层返回错误增量。注意，该方法不适用参数“z”。它包含在参数中，以便使接口与其他类的delta一致（旧版中的delta方法）
        """
        return (a - y)
...
class Network(object):
    ...
    def default_weight_initializer(self):
        """
        使用均值为0，标准差为1/sqrt(n)的高斯分布（正态分布）初始化每个位于连接到同一个神经元的权重数的平方根上的权重和偏置。
        注意，第一层为输入层，不初始化偏置。
        """
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        注意，如果未设置相应标志，列表为空。
        """
        # early stopping functionality(现在不用):
        best_accuracy = 1
    ...
    def backprop(self, x, y):
        ...
        """
        这里，l=1表示神经元的最后一层，l=2表示第二层，以此类推。
        将它重新编号而不是采用推导时候的编号是为了利用Python可以在列表中使用负索引这一事实。
        """
    ...
    def getResults(self, data):
        results = [np.argmax(self.feedforward(x[0])) for x in data]
